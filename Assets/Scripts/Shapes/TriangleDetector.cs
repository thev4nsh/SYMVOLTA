...
namespace SYMVOLTA.Shapes
{
    public static class TriangleDetector
    {
        public static float CalculateAccuracy(List<Vector2> points)
        {
            ...
            float cornerScore = corners == 3 ? 1f : Mathf.Max(0f, 1f - Mathf.Abs(corners - 3) * 0.3f);
            float angleScore = CalcAngleScore(points);
            float accuracy = (cornerScore * 0.5f) + (angleScore * 0.5f);
        ...
        private static float CalcAngleScore(List<Vector2> points)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                totalDeviation += Mathf.Abs(angle - 60f); // Ideal equilateral
            }
            return Mathf.Clamp01(1f - (totalDeviation / 180f));
        }
    }
}
